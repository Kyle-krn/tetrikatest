...
def task_3(data: dict) -> int:
    events = []
    for name, times in data.items():
        for indx in range(len(times)):
            event = (name, times[indx], 1- 2*(indx % 2))
            events.append(event)
    events.sort(key=itemgetter(1, 2))
    start_time = events[0][1]
    counter_actor = {"lesson": 0, "pupil": 0, "tutor": 0}
    count_actor = 0
    total_time = 0

    for event in events:
        if count_actor == 3:
            total_time += event[1] - start_time
        counter_actor[event[0]] += event[2]
        count_actor = sum([1 for i in counter_actor.values() if i > 0])
        start_time = event[1]
